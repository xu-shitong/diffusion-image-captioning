...
Original file is located at
    https://colab.research.google.com/drive/1NzML1VBkduOfD7Ab6MNrWZHZGrj-2AEC
...
flickr30k_image = torch.load("flickr30k_clip_image.pickle").to(device)
flickr30k_text = torch.load("flickr30k_clip_text.pickle").to(device)
...
train_dataset = FlickrCLIPDataset(pd.read_csv("captions.txt")["caption"], tokenizer)
...
# trial on inference
model = torch.load(f"batch8_maxlen16_round3E-01_lossseries_sum_batch_average_clipconcat_clipmask10_train-embedFalse_samplesize100_x_0_predictFalse.pickle").to(device)
model.model.add_module("activation", activations.GELUActivation())
model.eval()
idx = 0
captions = pd.read_csv("captions.txt")
image = Image.open("flickr8k/images/" + captions["image"].loc[idx])
plt.imshow(image)
plt.show()
...
# multi-step inference
for _ in range(10):
  restored = torch.rand_like(x_0, device=device)
  for i in range(5):
    out, restored = model(restored[:, :MAX_LENGTH, :], image_clip, torch.zeros_like(image_clip), mask)

  print("inferred: ", train_dataset.tokenizer.decode(nn.functional.softmax(out, dim=-1).argmax(dim=-1)[0]))
...
file_names = [
  "batch8_maxlen16_round3E-01_lossseries_sum_batch_average_clipconcat_clipmask10_train-embedFalse_samplesize100_x_0_predictFalse",
  "batch8_maxlen16_round3E-01_lossseries_sum_batch_average_clipconcat_clipmask11_train-embedFalse_samplesize100_x_0_predictFalse",
  # "batch8_maxlen16_round3E-01_lossseries_sum_batch_average_clipconcat_clipmask10_train-embedFalse_samplesize100",
]

fig, axarr = plt.subplots(len(file_names), 4, figsize=(5, 5))
for i, file_name in enumerate(file_names):
  losses = extract_float(file_name + ".txt", start="epoch") # shape [batch size, 7] each line is [batch number, x_t, x_1, prob losses, validation x_t x_1 prob losses]
...
fig, axarr = plt.subplots(2, 3, figsize=(2, 3))
axarr[0, 1].set_title("str")
axarr[0, 1].plot(torch.ones((2,3)))
